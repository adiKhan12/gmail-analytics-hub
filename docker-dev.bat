@echo off
echo === Email Planner Docker Development Setup ===
...
if %ERRORLEVEL% equ 0 (
    ...
    echo The Email Planner development environment is now running!
    echo Access the application at: http://localhost:5173
    echo.
    echo Useful commands:
    echo   docker-compose -f docker-compose.dev.yml logs -f        # View logs
    echo   docker-compose -f docker-compose.dev.yml down           # Stop the application
    echo   docker-compose -f docker-compose.dev.yml restart        # Restart the application
) else (
